...
static uint64_t convert_flags_x64(uint32_t common_flags) 
{
    ...
    if (common_flags & PAGE_FLAG_PRESENT)   x64_flags |= PAGE_PRESENT;
    if (common_flags & PAGE_FLAG_WRITABLE)  x64_flags |= PAGE_WRITE;
    if (common_flags & PAGE_FLAG_USER)      x64_flags |= PAGE_USER;
...
uintptr_t arch_virt_to_phys(uintptr_t virt_addr) 
{
    ...
    if (virt_addr < 0x200000) {  // Primeros 2MB
        return virt_addr;
    }
