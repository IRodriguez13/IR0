...
bool ps2_mouse_read_packet(ps2_mouse_packet_t *packet)
{
    ...
    packet->flags = 0;
    if (mouse_state.left_button) packet->flags |= PS2_MOUSE_LEFT_BUTTON;
    if (mouse_state.right_button) packet->flags |= PS2_MOUSE_RIGHT_BUTTON;
    if (mouse_state.middle_button) packet->flags |= PS2_MOUSE_MIDDLE_BUTTON;
    
    packet->delta_x = 0; // Would be calculated from movement
    packet->delta_y = 0;
    packet->delta_wheel = mouse_state.wheel;
    packet->extra_buttons = 0;
    if (mouse_state.button4) packet->extra_buttons |= 0x01;
    if (mouse_state.button5) packet->extra_buttons |= 0x02;
...
void ps2_mouse_process_packet(const ps2_mouse_packet_t *packet)
{
    ...
    if (mouse_state.x > 1023) mouse_state.x = 1023;
    if (mouse_state.y > 767) mouse_state.y = 767;
...
bool ps2_mouse_enable_wheel(void)
{
    ...
    if (!ps2_mouse_set_sample_rate(200)) return false;
    if (!ps2_mouse_set_sample_rate(100)) return false;
    if (!ps2_mouse_set_sample_rate(80)) return false;
