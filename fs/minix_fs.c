...
minix_inode_t *minix_fs_find_inode(const char *pathname)
{
  ...
  // Si no hay disco real, crear inodes virtuales
  if (!ata_is_available())
  {
    static minix_inode_t virtual_inode;
    virtual_inode.i_mode =
        MINIX_IFDIR | MINIX_IRWXU | MINIX_IRGRP | MINIX_IROTH;
    virtual_inode.i_uid = 0;
    virtual_inode.i_size = 1024;
    virtual_inode.i_time = 0;
    virtual_inode.i_gid = 0;
    virtual_inode.i_nlinks = 2;
    return &virtual_inode;
  }
...
int minix_fs_init(void)
{
  ...
  if (!ata_is_available())
  {
    // Crear un filesystem virtual básico como fallback
...
int minix_fs_format(void)
{
  ...
  if (minix_write_block(minix_fs.superblock.s_imap_blocks + 1, inode_block) !=
      0)
  {
    return -1;
  }
...
int minix_fs_ls(const char *path)
{
  ...
  // Si no hay disco real, usar filesystem virtual
  if (!ata_is_available())
  {
    // Usar syscall write para que la salida vaya al shell
    extern int64_t sys_write(int fd, const void *buf, size_t count);
    if (target_path[0] == '/' && target_path[1] == '\0')
    {
      sys_write(1, "drwxr-xr-x  2 root root  4096 Jan 10 12:00 .\n", 46);
      sys_write(1, "drwxr-xr-x  2 root root  4096 Jan 10 12:00 ..\n", 47);
      sys_write(1, "drwxr-xr-x  2 root root  4096 Jan 10 12:00 bin\n", 48);
      sys_write(1, "drwxr-xr-x  2 root root  4096 Jan 10 12:00 etc\n", 48);
      sys_write(1, "drwxr-xr-x  2 root root  4096 Jan 10 12:00 home\n", 49);
      sys_write(1, "drwxr-xr-x  2 root root  4096 Jan 10 12:00 tmp\n", 48);
      sys_write(1, "-rw-r--r--  1 root root   256 Jan 10 12:00 readme.txt\n",
                55);
    }
    else
    {
...
int minix_fs_cat(const char *path)
{
  ...
  {
    extern int64_t sys_write(int fd, const void *buf, size_t count);
    if (path[0] == 'r' && path[1] == 'e') // readme.txt
    {
      sys_write(1, "Welcome to IR0 Kernel!\n", 23);
      sys_write(1, "This is a simple operating system kernel.\n", 42);
      sys_write(
          1, "Available commands: ls, cat, mkdir, touch, ps, help, exit\n", 58);
    }
    else
    {
      sys_write(1, "This is a virtual file: ", 24);
      sys_write(1, path, strlen(path));
      print("\n");
      print("Content would be here if this was a real file.\n");
    }
    print("--- End of file ---\n");
    return 0;
  }
