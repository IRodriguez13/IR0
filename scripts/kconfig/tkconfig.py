...
class KernelConfigGUI:
    def __init__(self, root):
        ...
        # Generate initial dynamic Makefile
        self.update_dynamic_makefile()
    ...
    def save_config_silent(self):
        """Save configuration to .config file without showing messageboxes (thread-safe)"""
        try:
            ...
            # Create autoconf.h
            os.makedirs('includes/generated', exist_ok=True)
            with open('includes/generated/autoconf.h', 'w') as f:
    ...
    def save_config(self):
        ...
        if self.save_config_silent():
            ...
            self.status_var.set("Configuration saved - Build button enabled")
            self.append_console("✓ Configuration saved to .config and includes/generated/autoconf.h\n")
            messagebox.showinfo("Success", "Configuration saved to .config and includes/generated/autoconf.h\n\nBuild button is now enabled.")
            return True
        else:
            self.append_console("✗ Failed to save configuration\n")
            messagebox.showerror("Error", "Failed to save configuration")
            return False
    
    def append_console(self, text):
        """Append text to console (thread-safe)"""
        self.root.after(0, lambda: self._append_console_sync(text))
    
    def _append_console_sync(self, text):
    ...
    def on_subsystem_toggle(self, subsystem_id):
        ...
        # Update dynamic Makefile in background
        self.update_dynamic_makefile()
    ...
    def save_config_silent(self):
        """Save configuration to .config file without showing messageboxes"""
        try:
            with open('.config', 'w') as f:
                f.write("# IR0 Kernel Configuration\n")
                f.write("# Generated by IR0 Kernel Config Tool\n")
    ...
    def update_dynamic_makefile(self):
        """Update dynamic Makefile based on current configuration"""
        def update_thread():
            try:
                script_dir = os.path.dirname(os.path.abspath(__file__))
                ...
                # Save config silently (without messageboxes) to ensure it's up to date
                self.save_config_silent()
                
                # Generate Makefile using Python script (more reliable than C library)
                ...
                if os.path.exists(generate_script):
                    ...
                    if result.returncode == 0:
                        self.root.after(0, lambda: self.status_var.set("✓ Makefile updated"))
                    else:
                        self.root.after(0, lambda: self.status_var.set(f"⚠ Makefile update failed"))
                elif self.build_wrapper:
                    # Fallback to C library if Python script not available
                    success, msg = self.build_wrapper.generate_makefile(
                        config_file, subsystems_json, arch, kernel_root
                    )
                    if success:
                        self.root.after(0, lambda: self.status_var.set("✓ Makefile updated"))
                    else:
                        self.root.after(0, lambda: self.status_var.set(f"⚠ Makefile update: {msg}"))
            except Exception as e:
    ...
    def setup_ui(self):
        ...
        self.build_button = ttk.Button(action_frame, text="Build Selected Subsystems", 
                                      command=self.on_build_selected, state='disabled')
        ...
        console_frame = ttk.LabelFrame(main_frame, text="Build Console", padding=5)
        console_frame.pack(fill='both', expand=False, pady=(5, 0))
        ...
        self.console_text.insert(tk.END, "Build console ready. Save configuration first, then build.\n")
        self.console_text.config(state=tk.DISABLED)
        
        # Status bar
        self.status_var = tk.StringVar()
        self.status_var.set("Ready - Save configuration to enable build")
        status_bar = ttk.Label(main_frame, textvariable=self.status_var, relief=tk.SUNKEN, 
                             anchor=tk.W, padding=3)
        status_bar.pack(fill='x', pady=(5, 0))
        
        self.current_subsystem = None
        self.config_saved = False  # Track if config has been saved
    ...
    def on_build_selected(self):
        """Build all selected subsystems using dynamic Makefile"""
        if not self.config_saved:
            messagebox.showwarning("Configuration Not Saved", 
                                 "Please save configuration first before building.")
            return
        ...
        if not messagebox.askyesno("Build Subsystems", 
                                  f"Build {len(selected_subsystems)} subsystem(s) using dynamic Makefile?"):
            return
        
        self.status_var.set("Building subsystems...")
        self.build_button.config(state='disabled')  # Disable during build
        
        def build_all_thread():
            script_dir = os.path.dirname(os.path.abspath(__file__))
            kernel_root = os.path.dirname(os.path.dirname(script_dir))
            
            # Save config silently and generate Makefile synchronously in this thread
            self.save_config_silent()
            
            # Generate Makefile synchronously using Python script (more reliable)
            script_dir = os.path.dirname(os.path.abspath(__file__))
            kernel_root = os.path.dirname(os.path.dirname(script_dir))
            config_file = os.path.join(kernel_root, '.config')
            subsystems_json = os.path.join(script_dir, 'subsystems.json')
            ...
            if os.path.exists(generate_script):
                ...
                # Fallback to C library
                self.build_wrapper.generate_makefile(
                    config_file, subsystems_json, arch, kernel_root
                )
            ...
            try:
                # Use dynamic Makefile build via subprocess to capture output
                ...
                if os.path.exists(makefile_path):
                    ...
                    # Show output in a separate window
                    self.root.after(0, lambda: self.show_build_output(process, len(selected_subsystems)))
                    
                    # Wait for process to complete
                    process.wait()
                    success = process.returncode == 0
                    if not success:
                        error_msg = "Build failed - see build output window for details"
                else:
                    # Fallback: Makefile doesn't exist, try alternative methods
                    if self.build_wrapper:
                        # Try using C library wrapper (but won't capture output)
                        success, msg = self.build_wrapper.build_dynamic(kernel_root)
                        if not success:
                            error_msg = msg
                    else:
                ...
                if success:
                    ...
                    self.root.after(0, lambda: messagebox.showerror("Build Failed", 
                        f"Build failed. Check the build output window for details."))
            except Exception as e:
                self.root.after(0, lambda: self.status_var.set(f"Error: {e}"))
                self.root.after(0, lambda: messagebox.showerror("Error", f"Failed to build: {e}"))
            finally:
    ...
    def on_clean_selected(self):
        """Clean object files for selected subsystems using dynamic Makefile"""
        selected_subsystems = [sid for sid in self.subsystems_data.keys() 
                              if self.config.get(f"SUBSYSTEM_{sid}", 'n') == 'y']
        ...
        if not messagebox.askyesno("Clean Subsystems", 
                                  f"Clean object files for {len(selected_subsystems)} subsystem(s) using dynamic Makefile?"):
            return
        ...
        def clean_all_thread():
            ...
            try:
                # Use dynamic Makefile clean
                if self.build_wrapper:
                    success, msg = self.build_wrapper.clean_dynamic(kernel_root)
                    if not success:
                        error_msg = msg
                else:
                    ...
                    if os.path.exists(makefile_path):
                        ...
                        error_msg = "Dynamic Makefile not found"
                
                # Show results
                if success:
                    self.root.after(0, lambda: self.status_var.set(
                        f"✓ Cleaned {len(selected_subsystems)} subsystem(s) successfully"))
                    self.root.after(0, lambda: messagebox.showinfo("Clean Success", 
                        f"Cleaned object files for {len(selected_subsystems)} subsystem(s) successfully!"))
                else:
                    self.root.after(0, lambda: self.status_var.set("✗ Clean failed"))
                    self.root.after(0, lambda: messagebox.showerror("Clean Failed", 
                        f"Clean failed:\n\n{error_msg}\n\n"
                        f"Check the console output for more details."))
            except Exception as e:
                self.root.after(0, lambda: self.status_var.set(f"Error: {e}"))
    ...
    def show_build_output(self, process, num_subsystems):
        """Show build compilation output in a separate window"""
        output_win = tk.Toplevel(self.root)
        output_win.title("Build Output")
        output_win.geometry("1000x700")
        
        # Header frame
        header_frame = ttk.Frame(output_win, padding=5)
        header_frame.pack(fill='x')
        
        ttk.Label(header_frame, text=f"Building {num_subsystems} subsystem(s)", 
        ...
        text = scrolledtext.ScrolledText(text_frame, wrap=tk.WORD, font=('Courier', 9),
                                        bg='#1e1e1e', fg='#d4d4d4',
                                        insertbackground='#ffffff')
        text.pack(expand=True, fill='both')
        
        text.insert(tk.END, f"=== Building {num_subsystems} subsystem(s) ===\n")
            ...
            try:
                if process.poll() is None:
                    ...
                    if line:
                        ...
                        output_win.after(50, update_output)
                    else:
                        output_win.after(50, update_output)
                else:
                    # Process finished
                    ...
                    if process.returncode == 0:
                        text.insert(tk.END, f"✓ Build completed successfully!\n")
                        text.insert(tk.END, f"✓ Built {num_subsystems} subsystem(s)\n")
                    else:
                        text.insert(tk.END, f"✗ Build failed with return code {process.returncode}\n")
                    text.see(tk.END)
                    text.update_idletasks()
            except Exception as e:
        ...
        def on_close():
            if process.poll() is None:
                if messagebox.askyesno("Build Running", "Build is still running. Terminate process?"):
                    process.terminate()
                    try:
