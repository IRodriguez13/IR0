...
void boot_main(void)
{
	...
	/* Exercise a non-QEMU L1 window and a 2 MiB boundary without touching it. */
	if (arm64_mmu_map_device_range(0x1001ff000UL, 0x3000UL) == 0)
		ir0_boot_smoke("ARM64_MMIO_HIGH_OK");
	else
		ir0_boot_smoke("ARM64_MMIO_HIGH_FAIL");
